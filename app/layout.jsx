...
export default async function RootLayout({ children }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      // Apply RTL positioning fixes for Arabic
      if (isRtl) {
        const applyRTLFixes = () => {
          // Find all elements with inline positioning styles and flip them
          const shapesWithLeft = document.querySelectorAll('.shape[style*="left:"]');
          const shapesWithRight = document.querySelectorAll('.shape[style*="right:"]');
          
          shapesWithLeft.forEach(element => {
            const style = element.getAttribute('style');
            if (style) {
              const leftMatch = style.match(/left:\s*([^;]+)/);
              if (leftMatch) {
                const leftValue = leftMatch[1];
                const newStyle = style.replace(/left:\s*[^;]+/, `right: ${leftValue}`);
                element.setAttribute('style', newStyle);
              }
            }
          });
          
          shapesWithRight.forEach(element => {
            const style = element.getAttribute('style');
            if (style) {
              const rightMatch = style.match(/right:\s*([^;]+)/);
              if (rightMatch) {
                const rightValue = rightMatch[1];
                const newStyle = style.replace(/right:\s*[^;]+/, `left: ${rightValue}`);
                element.setAttribute('style', newStyle);
              }
            }
          });
          
          // Handle cards with positioning
          const cardsWithPositioning = document.querySelectorAll('.card[style*="right:"], .card[style*="left:"]');
          cardsWithPositioning.forEach(element => {
            const style = element.getAttribute('style');
            if (style) {
              let newStyle = style;
              const leftMatch = style.match(/left:\s*([^;,}]+)/);
              ...
              if (leftMatch) {
                newStyle = newStyle.replace(/left:\s*[^;,}]+/, `right: ${leftMatch[1]}`);
              }
              if (rightMatch) {
                newStyle = newStyle.replace(/right:\s*[^;,}]+/, `left: ${rightMatch[1]}`);
              }
        ...
        setTimeout(applyRTLFixes, 100);
        
        // Also apply fixes when new content loads (for dynamic content)
        const observer = new MutationObserver(() => {
          setTimeout(applyRTLFixes, 50);
        });
